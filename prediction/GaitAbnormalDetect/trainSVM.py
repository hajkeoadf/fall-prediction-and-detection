...
from sklearn.svm import SVC
from sklearn.metrics import accuracy_score, confusion_matrix
from joblib import dump  # 导入用于保存模型的函数
...
# 假设数据列按照如下顺序排列：特征1, 特征2, ..., 特征N, 标签
# 根据实际数据列进行调整
feature_columns = df.columns[1:-1]  # 获取除了最后一列的所有列名作为特征列
label_column = df.columns[-1]      # 最后一列为标签列
...
# 创建SVM模型，使用RBF核
svm_classifier = SVC(kernel='rbf')  # 使用RBF核函数
...
dump(svm_classifier, model_file_path)  # 使用joblib保存模型
